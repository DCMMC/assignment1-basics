...
if __name__ == "__main__":
    ...
    if corpus_path and corpus_path.exists():
        freq_table = pre_tokenize_file(
            corpus_path, desired_num_chunks=1, special_tokens=["<|endoftext|>"]
        )
        num_merges = min(243, max(0, 500 - 1 - 256))
    else:
        freq_table = Counter({
            b"low": 50, b"lower": 20, b"widest": 30, b"newest": 60,
            b"hello": 100, b"world": 80, b"the": 200, b"and": 150,
        })
        num_merges = 20

    versions = [
        ("original (list + Counter, full pair_counts each round)", find_merges_original),
        ("optimization 1 (linked list + dict, full pair_counts each round)", find_merges_linked_list),
        ("optimization 2 (linked list + incremental pair_counts)", find_merges),
    ]
    results = []
    for name, fn in versions:
        t0 = time.perf_counter()
        merges = fn(freq_table, num_merges)
    ...
    print(f"  (freq_table size={len(freq_table)}, num_merges={num_merges})")